...
impl AnagramSolver {
    ...
    fn backtrack(
        ...
        if let Some(timeout_sec) = constraints.timeout_seconds {
            if internal_state.start_time.elapsed().as_secs_f64() > timeout_sec {
                ...
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "  Pruned: Timeout triggered NOW. Path: {:?}", current_path
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                return;
            }
        ...
        if let Some(max_sol) = constraints.max_solutions {
            if internal_state.solutions_found_count >= max_sol { 
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "  Pruned: Max solutions reached. Path: {:?}", current_path
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                return; 
            }
        ...
        if let Some(max_w) = constraints.max_words {
            if current_path.len() > max_w { 
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "  Pruned: Path len {} > max_words {}. Path: {:?}", current_path.len(), max_w, current_path
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                return; 
            }
        ...
        if remaining_counts.is_empty() {
            if let Some(file) = log_file.as_deref_mut() { writeln!(file, "  REACHED BASE CASE (remaining_counts is empty). Path: {:?}", current_path
                ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
            }
            if !current_path.is_empty() {
                ...
                if let Some(satisfied_mask) = &internal_state.patterns_satisfied_mask {
                    ...
                    if let Some(file) = log_file.as_deref_mut() { 
                        writeln!(file, "    No pattern constraint active in base case. Path: {:?}", current_path
                            ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                    }
                }
                ...
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "    Attempting to insert solution: {:?}", solution_candidate
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                if solutions_set.insert(solution_candidate.clone()) { 
                    internal_state.solutions_found_count += 1;
                    if let Some(file) = log_file.as_deref_mut() { 
                        writeln!(file, "    Solution ADDED. New count: {}. Set size: {}. Path: {:?}", internal_state.solutions_found_count, 
                        solutions_set.len(), current_path).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                    }
                    if let Some(max_sol) = constraints.max_solutions {
                        if internal_state.solutions_found_count >= max_sol { 
                            if let Some(file) = log_file.as_deref_mut() { 
                                writeln!(file, "    Max solutions reached after adding. Path: {:?}", current_path
                                    ).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
                            }
                            return; 
                        }
                ...
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "    Current path IS EMPTY in base case. Path: {:?}", current_path
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
                }
            }
            if let Some(file) = log_file.as_deref_mut() { 
        ...
        if let Some(file) = log_file.as_deref_mut() { 
            writeln!(file, "  Did not hit base case (remaining_counts_total = {}). Path: {:?}", remaining_counts.total(), current_path
                ).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
        }
        ...
        if remaining_counts.total() < self.trie.get_min_word_len() { 
            if let Some(file) = log_file.as_deref_mut() { 
                writeln!(file, "  Pruned: remaining_total {} < min_dict_word_len {}. Path: {:?}", 
                    remaining_counts.total(), self.trie.get_min_word_len(), current_path
                    ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
            }
            return; 
        }
        ...
        if let Some(min_len) = constraints.min_word_length {
            if !current_path.is_empty() && remaining_counts.total() < min_len { 
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "  Pruned: remaining_total {} < min_word_len {}. Path: {:?}", remaining_counts.total(), min_len, current_path
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                return; 
            }
        ...
        if let Some(max_w) = constraints.max_words {
            if current_path.len() == max_w && !remaining_counts.is_empty() { 
                if let Some(file) = log_file.as_deref_mut() {
                    writeln!(file, "  Pruned: Path len {} == max_words {} but letters remain. Path: {:?}", current_path.len(), max_w, current_path
                        ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                return; 
            }
        ...
        if let Some(file) = log_file.as_deref_mut() { 
            writeln!(file, "  Calling find_one_word_recursive for path: {:?}", current_path
                ).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
        }
        ...
        if let Some(file) = log_file.as_deref_mut() { 
            writeln!(file, "BACKTRACK EXIT: path={:?}", current_path).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
        }
    }
    ...
    fn find_one_word_recursive(
        ...
        if word_so_far == "eleven" { // Example specific debug
            if let Some(file) = log_file.as_deref_mut() {
                writeln!(file, "    FOWR: word_so_far IS 'eleven'. current_trie_node.is_end_of_word = {}", 
        ...
        if internal_state.timed_out { 
            if let Some(file) = log_file.as_deref_mut() { 
                writeln!(file, "    FOWR Pruned: Timed out.").unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
            } 
            return; 
        }
        if let Some(max_sol) = constraints.max_solutions {
            if internal_state.solutions_found_count >= max_sol { 
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "    FOWR Pruned: Max solutions.").unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                } 
                return; 
            }
        }
        ...
        if current_trie_node.is_end_of_word && !word_so_far.is_empty() {
            if let Some(file) = log_file.as_deref_mut() { 
                writeln!(file, "    FOWR: Found candidate word: '{}'. Path before push: {:?}", word_so_far, path
                    ).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
            }
            ...
            if passes_min_length {
                ...
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "      FOWR: '{}' passes min_length (len {}). Path: {:?}", 
                    word_so_far, word_so_far.len(), path).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
                ...
                if let Some(patterns_to_satisfy) = &constraints.contains_patterns {
                    if let Some(current_mask) = internal_state.patterns_satisfied_mask.as_mut() {
                        if let Some(file) = log_file.as_deref_mut() { 
                            writeln!(file, "      FOWR: Checking patterns. Word: '{}'. Mask before: {:?}", word_so_far, current_mask
                                ).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                        }
                ...
                self.backtrack(path, current_overall_counts, &self.trie.root, constraints, 
                    solutions_set, internal_state, log_file.as_deref_mut());
                
                path.pop();

                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "      FOWR: Popped '{}'. Path is now: {:?}", word_so_far, path).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
                }
                ...
                if let Some(current_mask) = internal_state.patterns_satisfied_mask.as_mut() {
                    ...
                    if !current_mask.is_empty() && log_file.is_some() && !original_mask_states_for_changed_indices.is_empty() { // only log if changes were made
                        writeln!(log_file.as_deref_mut().unwrap(), "      FOWR: Mask after revert: {:?}", current_mask
                            ).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
                    }
                }
                ...
                if let Some(file) = log_file.as_deref_mut() { 
                    writeln!(file, "      FOWR: '{}' FAILED min_length (len {}). Path: {:?}",
                    word_so_far, word_so_far.len(), path).unwrap_or_else(|e| eprintln!("Log write error: {}", e)); 
                }
            }  
        }
        
            if word_so_far.len() >= self.trie.max_word_len { // Use >= because if len == max_word_len, can't extend
                if word_so_far.len() > self.trie.max_word_len { // Log only if strictly greater
                    if let Some(file) = log_file.as_deref_mut() { 
        ...
        for (key_ref_char_code, value_ref_next_node) in current_trie_node.children.iter() {
            ...
            if current_overall_counts.get(ch).unwrap_or(0) > 0 {
                ...
                self.find_one_word_recursive(
                    ...
                    constraints, solutions_set, internal_state, log_file.as_deref_mut(), 
                );
        ...
        if let Some(file) = log_file.as_deref_mut() { 
            writeln!(file, "  FOWR EXIT: word_so_far='{}'", 
                word_so_far).unwrap_or_else(|e| eprintln!("Log write error: {}", e));
        }
    }
}